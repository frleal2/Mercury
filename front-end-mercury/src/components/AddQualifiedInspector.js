...
function AddQualifiedInspector({ isOpen, onClose, onInspectorAdded }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(`${BASE_URL}/api/qualified-inspectors/`, submitData, {
        headers: { 'Authorization': `Bearer ${session.accessToken}` }
      });

      onInspectorAdded();
    } catch (error) {
      console.error('Error adding qualified inspector:', error);
      if (error.response?.status === 401) {
        await refreshAccessToken();
      }
      setError(error.response?.data?.error || 'Failed to add qualified inspector');
    } finally {
      setLoading(false);
    }
  };
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                    <div className="flex items-start">
                      ...
                      <div className="text-sm text-blue-800">
                        <p className="font-medium">CFR 396.19 Compliance</p>
                        <p className="mt-1">
                          This inspector will be authorized to perform annual vehicle inspections as required by CFR 396.17.
                        </p>
                      </div>
                    </div>
                  </div>
...
export default AddQualifiedInspector;