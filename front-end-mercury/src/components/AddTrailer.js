...
function AddTrailer({ onClose, onTrailerAdded }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const response = await axios.post(`${BASE_URL}/api/trailers/`, formData, {
        headers: {
      ...
      if (error.response && error.response.status === 401) {
        ...
        if (newAccessToken) {
          try {
            ...
            console.error('Error retrying add trailer:', retryError);
            if (retryError.response?.data) {
              setErrors(retryError.response.data);
            } else {
              alert('Failed to add trailer after refreshing token.');
            }
          }
        } else {
      ...
      <div className="relative top-10 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 xl:w-1/3 shadow-lg rounded-md bg-white max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-gray-200">
            ...
            >
              {submitting ? 'Adding Trailer...' : 'Add Trailer'}
            </button>
          </div>
        </form>
      </div>
