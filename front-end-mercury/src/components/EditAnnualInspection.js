...
function EditAnnualInspection({ isOpen, onClose, inspection, onInspectionUpdated }) {
  ...
  const populateFormData = () => {
    if (inspection) {
      setFormData({
        vehicle_type: inspection.truck ? 'truck' : 'trailer',
        truck: inspection.truck || '',
        trailer: inspection.trailer || '',
        inspection_date: inspection.inspection_date || '',
        inspector_name: inspection.inspector_name || '',
        inspection_facility: inspection.inspection_facility || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.patch(`${BASE_URL}/api/annual-inspections/${inspection.id}/`, submitData, {
        headers: { 'Authorization': `Bearer ${session.accessToken}` }
      });
      ...
      setError(error.response?.data?.error || 'Failed to update annual inspection');
    } finally {
      setLoading(false);
    }
  };
...
export default EditAnnualInspection;